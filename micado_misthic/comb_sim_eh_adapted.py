...
flux_dir                    = tristan_main_path+'INPUT/Flux_input/'
### directory with simulated spectrum data
spec_dir                    = tristan_main_path+ 'INPUT/R500_cloudless_2025/'
### directories with the coronagraphic simulated images
coro_sim_dir_prefix         = f'D:/NEW_PHASE2021/{coronagraph}_{quartile}_NoNCPA/'
planet_sim_dir_prefix       = f'/media/ehuby/T7/OUTPUT/planet_sims/{coronagraph}_{quartile}_NoNCPA/'
coro_resized_dir_prefix     = tristan_main_path + f'output/misthic/Simu30min/resized_fft/{coronagraph}_{quartile}_noNCPA/'
planet_resized_dir_prefix   = f'/media/ehuby/T7/OUTPUT/resized_fft_planet/{coronagraph}_{quartile}_noNCPA/'
### directories where the resulting ADI images are saved
adi_img_dir_prefix          =tristan_main_path + f'output/misthic/Simu30min/ADI/{star_name}/{coronagraph}_{quartile}_noNCPA/'
pladi_img_dir_prefix        = f'/home/ehuby/WORK/SIMU/SIMU_MICADO/ANALYSIS/PLANETS/{star_name}/{coronagraph}_{quartile}_noNCPA/'
### telescope pupil file, needed to normalize the input flux
telescope_pupil_file        = tristan_main_path + '/INPUT/PUPIL/Pupil_ELT_v03.fits'
...
for i in range(len(lbd)):
    coro_sim_dir   = coro_sim_dir_prefix + f'{coronagraph}_25Hz_NEW_noNCPA_Lambda={lbd[i]:5.3f}/'
    ...
    if coro_on == True:
        ...
        psf_adi_file = adi_img_dir+f'{star_name}_psf_adi_{star_mag}mag_{dist_pc}pc_{ron}e-ron_tint{frame_exp_time}sec.fits'
        ...
        if len(glob.glob(coro_adi_file))==1 :
            ...
            adi_sum_coro, psf_sum_coro = micado_adi(coro_cube, psf_cube, coro_sim_dir)
        
            ### Convolution
            print('----------')
            print("Beginning coro image convolution")
            # dim = len(adi_sum_coro[0,:])
            # mask = cheese_func(dim, lbd[i], r=0.5, px_scale=1.5)
        ...
        plt.title(r'Coronagraph only - No planet',fontsize=10)
        ...
        # Save the contrast curve plot
        if not os.path.exists(adi_img_dir):
            os.makedirs(adi_img_dir)
        save_file = f'{star_name}_coro_contrast_{coronagraph}_{quartile}_lbd{lbd[i]:5.3f}_pxscale{pixscale_in_mas}_Mag{star_mag}'
    ...
    if coro_on == True:
        ...
        if save == True:
            ...
            hdu_coro.writeto(coro_adi_dir + 'coro_adi_mag={}.fits'.format(star_mag),overwrite=True)
