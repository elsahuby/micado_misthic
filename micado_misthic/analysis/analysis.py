...
def get_radial_minmax_profile(img, nbin=1, centering='FFTSTYLE', cyx=None, mask=None):
    """
    Computes the mean radial profile of the image.
    ...
    O       = np.ones((ny,nx))
    Z       = np.zeros((ny,nx))
    profile_min = np.zeros(npts)
    profile_max = np.zeros(npts)
    xvector = np.zeros(npts)
    ...
    for k in range(npts):
        ...
        npix = np.sum(M)
        if npix > 0 :
            profile_min[k] = np.min(img[ind_M])
