...
This script processes the MISTHIC simulation files using the following steps : 
        1) Resizing of the images, PSF and perfect PSF to match MICADO's spatial sampling rate (1.5 or 4 mas/pixel) (function : fftresize)
        2) Calculate the photon flux through MICADO's filters depending on star/planet magnitude (function : get_micado_flux)
        3) Adding the photon and readout noise (function : scale_to_photon)
        4) Applying the ADI process to the images and the PSF (substracting the mean of all images to every image, derotating the field with 
         the parallactic angle table (function : get_parallactic_angle), summing every image)
        5) Calculating the contrast curve/radial profile of the ADI processed image (function : get_rms_contrast)
        5) Displaying the ADI images and the matching contrast curve (with the planet(s) placed relative to the curve) at every lbd

It is possible to save the files at every step of this process using the Switches. The directories must be changed by hand at the beginning.
@author: hbaran
...
###-- Wavelength table
lbd             = [1.190, 1.270, 1.582, 1.693, 2.100, 2.235]# central wavelength of J1, J2, H1, H2, K1, K2 bands in um
#lbd             = [1.245, 1.635, 2.145] # central wavelength of J, H, K bands in um


###-- Observation params
coronagraph     = 'CLC1' # 'CLC0' 'CLC1' or 'CLC2'
quartile        = 'MED'#['Q1', 'Q4', 'MED']
frame_exp_time  = 10. ## exposure time per frame in seconds ### NEW
ron             = 15 # electrons; from FDR paper
#pixscale_in_mas = lbd[0]* 1e-6/tel_diam * 180 / np.pi *3600 *1000 /2# 1.5 or 4 mas
pixscale_in_mas = 1.5
tel_diam        = 38.542 # in m

###-- Simulation parameters
sim_duration    = [30.0] # in minutes
n_images        = sim_duration[0] * 6

# Switches
save            = False
coro_on         = True  ### process the coro image
planet_on       = True  ### process the planet image
save_adi        = False ### save the ADI processed images
save_resized    = False ### save the resized images to be reused in future runs
new_resizing    = True  ### reuse or not the saved resized images ; must be switched to True for the first run, can be switched to False for 
                        # every run after save_resized has first been switched to True. 
save_noisy      = False

###-- Planet Parameters
pl_dist         = [31.000, 300.00]
r_pl            = 1 # jupiter masses
temp            = 800 # Kelvin
logg            = 5.0
met             = 3.16 # multiple of solar metallicity ; see ExoREM model
CO              = 0.5

###-- Star Parameters ; magnitudes are from the SIMBAD catalogue at https://simbad.cds.unistra.fr/simbad/

## HR8799 ###################################################################
# star_mag = [5.383, 5.28, 5.240] # magnitudes respectively in J, H, K # formerly 5.24 in K 
# star_name = 'HR8799'
...
### Eps Eridani ##############################################################
star_name       = 'EpsEri'
star_mag        = [ 2.23, 2.23, 1.75, 1.75, 1.67, 1.67] # [ 2.23, 1.75, 1.67] #formerly 1.75 in H
...
### AB Pictoris ##############################################################
# star_name = 'AB Pic'
# #star magnitudes in each band listed in lbd
# star_mag = [7.576, 7.576, 7.088, 7.088, 6.981, 6.981] #formerly 2.41
# zenith_distance = np.abs(-24.5 - (19.9)) # deg
# dist_pc = 50.12 # distance in parsecs
# airmass = 1. #/np.cos(zenith_distance*np.pi/180.)

### HD14082B #################################################################
# star_name = 'HD14082B'
# star_mag = [6.57, 6.355, 6.262] #formerly 2.41 
# zenith_distance = np.abs(-24.5 - (25.2)) # deg
# dist_pc = 39.6 # distance in parsecs
# airmass = 1. #/np.cos(zenith_distance*np.pi/180.)
...
### directories where the resized images are saved if save_resized == True
...
### directories where the resulting ADI images are saved if save_adi == True
...
fig, axs = plt.subplots(len(lbd), 2, sharex = 'col')

##############################################################################################################
for i in range(len(lbd)):
    ### path to the coro simulation files
    coro_sim_dir   = coro_sim_dir_prefix + f'lambda={lbd[i]:5.3f}/TURBU_ABERR/duree={sim_duration[0]}min/'
    ...
    sampling_misthic = np.float32(config['detectorconfig']['detector_sampling']) ## Sampling for initial simulation
    ...
    t = open(parallactic_angle_dir+f"parangle_tab_zenith_angle_{zenith_distance}_{n_images}_imagespercube.txt", "w")
    #par_angle_table = Table([parangle_tab],names=('a'))
    t.write(np.array2string(parangle_tab))
    t.close()
    print('Parallactic angles successfully computed : file saved to ', parallactic_angle_dir+f"parangle_tab_zenith_angle_{zenith_distance}_{n_images}_imagespercube.txt")
    ...
    if coro_on == True:
    #     # -----------------------------------------------------------------------------
        if new_resizing == True : ### resize the images so they match MICADO's spatial sampling rate (1.5 or 4 mas/pixel)

            print('-- Coro Img: Resizing')

            ### open the FITS files created by the MISTHIC simulator
            ...
            if save_resized == False : ### do not save the resized files ; the resizing will need to be redone for every run until save_resized is True
                coro_cube, coro_scale_coeff     = fft_resize(cube_coro, lbd[i], sampling_misthic, pixscale_in_mas)
                psf_cube,  psf_scale_coeff      = fft_resize(cube_psf,  lbd[i], sampling_misthic, pixscale_in_mas)
                perf_psf,  perf_psf_scale_coeff = fft_resize(perf_psf,  lbd[i], sampling_misthic, pixscale_in_mas)

            else : ### save the resized files in coro_resized_dir directory
                # If desired resizing directory doesn't exist, create it
                ...
                print('Coro resized files saved to ', coro_resized_dir)
        else: ## if new_resizing == False, reuse the previously saved resized images. This will not work if save_resized has never been True. 
        # Opening resized files
            print('Opening coro resized files from ', coro_resized_dir)
        ...
        if len(glob.glob(coro_adi_file))==1 :
            ...
            print('Coro ADI image file loaded : ', coro_adi_file)
            print('Coro ADI PSF file loaded', psf_adi_file)
            print('Coro ADI convoluted image file loaded : ', coro_adi_conv_file)
            print('Coro ADI convoluted PSF file loaded', psf_adi_conv_file)
            ...
            ### Get the star spectrum from the file in flux_dir ; star flux is in photon/m²/µm/s 
            _, star_flux = get_star_spectrum(flux_dir, star_mag[i], lbd[i])
            print('star_flux_type =', star_flux)
            # Aperture Surface
            telescope_surface = get_aperture_surface(telescope_pupil_file)
            print(f'lambda = {lbd[i]}')

            ### Getting the photon flux through MICADO's filters and the pupil ###
            ...
            print("Photons reaching detector in perf system (coro)  : {:e}".format(photon_flux))
            ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### 

            perf_psf_sum = np.sum(perf_psf)
            
            ### Adding the noise to the PSF cube and the image cube : photon noise, readout noise ###
            ...
            ### Toggle the swith to True if you want to save the noisy files ###
            if save_noisy == True :
                fits.writeto(adi_img_dir+'psf_cube_noisy.fits', psf_cube_noisy, overwrite = True)
                fits.writeto(adi_img_dir+'img_cube_noisy.fits', coro_cube_noisy, overwrite = True)

                print('Noisy files succesfully saved to ', adi_img_dir)
            ...
            ### Convolution with the mask of size lbd/D to get a more easily legible figure
            print('----------')
        ...
        print('max_psf_coro =', max_psf_coro)
        #print('norm_adi_coro =', norm_adi_coro)

        ### Creating contrast curve/radial profile for the processed image : 5-sigma contrast depending on distance to center in mas
    ...
    if planet_on == True:
        ...
        for j in range(len(pl_dist)):
            ...
            planet_sim_dir = planet_sim_dir_prefix+ f'lambda={lbd[i]:5.3f}/p_dist_mas={pl_dist[j]:5.3f}/TURBU_ABERR/duree={sim_duration[0]}min/'
            ...
            if len(glob.glob(pl_adi_file))==1 :
                ...
                adi_sum_pl = fits.getdata(pl_adi_file, overwrite = True)
                psf_sum_pl = fits.getdata(plpsf_adi_file, overwrite = True)
                conv_adi_pl = fits.getdata(pl_adi_conv_file, overwrite = True)
                conv_psf_pl = fits.getdata(plpsf_adi_conv_file, overwrite = True)
                ...
                # print('perf_file_pl =', perf_file_pl)
                ...
                ### Adding noise to the PSF cube and the image cube : photon noise, readout noise ###
                ...
                plpsf_cube_noisy, perf_psf_noisy, flux_per_frame = scale_to_photons(cube_psf_pl, perf_psf_pl, 
                                                                                    ...
                                                                                    no_noise=False)
                print('-----------')
                print('Noise successfully added to planet')

                print('max planet flux sans bruit :', np.max(planet_cube[0]))
                print('max planet flux avec bruit : ', np.max(planet_cube_noisy[0]))
                ...
                # Convolution with the mask of size lbd/D
                print('----------')
                ...
                print('adi_sum_pl.shape =', adi_sum_pl.shape)
                ...
                ### SAVE
                ### Toggle the switch to True if you want to save the ADI processed image and PSF of the planet
                if save_adi == True :
                    if not os.path.exists(pladi_img_dir):
                        os.makedirs(pladi_img_dir)
                    fits.writeto(pl_adi_file, adi_sum_pl, overwrite = True)
                    fits.writeto(plpsf_adi_file, psf_sum_pl, overwrite = True)
                    fits.writeto(pl_adi_conv_file, conv_adi_pl, overwrite = True)
                    fits.writeto(plpsf_adi_conv_file, conv_psf_pl, overwrite = True)
            ...
            pixscale = pixscale_in_mas/1000.
            ny, nx = conv_adi_coro.shape
            e = [-ny/2*pixscale - pixscale/2, nx/2*pixscale - pixscale /2, -ny/2*pixscale - pixscale/2, nx/2*pixscale - pixscale/2]

            ### ADI processed image of the coronograph added to the ADI processed image of the planet
            axs[i][0].imshow(conv_adi_coro + conv_adi_pl, cmap="viridis", origin='lower', 
                    extent = e)#, vmax = 10)
            #axs[i][0].set_title(f"{star_name}: {coronagraph} {quartile} ADI image at $\lambda$={lbd[i]:5.3f}, pxscale={pixscale_in_mas}, Mag {star_mag}",fontsize=12)
            #axs[i][0].set_title('Planet: lbd = {}, {} R_Jup, {} pc, {}K, logg={}, met={:.2f}, CO ratio={:3.2f}'.format(lbd[i], r_pl, dist_pc, temp, logg, met, CO),fontsize=10)
            axs[i][0].set_xlabel('[arcsec]')
            axs[i][0].set_ylabel('[arcsec]')
            axs[i][0].tick_params(axis = 'x', labelsize = 7)

            ### Contrast curve of the coronograph 
            axs[i][1].plot(x, rms_contrast, label="Lambda={:5.3f} µm".format(lbd[i]))
            #axs[i][1].suptitle(f"{star_name}: {coronagraph} {quartile} Contrast Profile of $\lambda$={lbd[i]:5.3f}, pxscale={pixscale_in_mas}, Mag {star_mag}",fontsize=12)
            #axs[i][1].set_title('Planet: lbd={}, {} R_Jup, {} pc, {}K, logg={}, met={:.2f}, CO ratio={:3.2f}'.format(lbd[i], r_pl, dist_pc, temp, logg, met, CO),fontsize=14)
            axs[i][1].set_title(f'Lambda = {lbd[i]}µm')
            axs[i][1].set_ylabel("Contrast, 5-sigma")
            axs[i][1].set_yscale('log')
            axs[i][1].grid(color='.9')

            ### Planet placed at its angular separation over the contrast curve
            axs[i][1].plot(p_dist_mas, max_planet, 'o')
axs[len(lbd)-1][1].set_xlabel("Distance au centre (millisecondes d'arc)", fontsize = 14)
...
fig.suptitle(f"{star_name}: {coronagraph} {quartile} ADI image, pxscale={pixscale_in_mas}",fontsize=18)

plt.show()
#plt.savefig(adi_img_dir+save_file+'_contrast.png')#, dpi=120, bbox_inches='tight')
#plt.savefig(f'D:/FROM_RED/flux_and_contrasts/Rapport/NEW_RADIALPROFILES/ADI_CONTRAST_star={star_name}_{coronagraph}_{quartile}_pxscl={pixscale_in_mas}_{len(pl_dist)}pl{temp}K.jpeg')


if save == True:
    # Save figure and coro adi images
    fig_dir = adi_img_dir_prefix
    print('adi_img_dir =', fig_dir)
...
'''fig, axs = plt.subplots(len(lbd), 2)
print('len lbd = ', len(lbd))
for i in range(len(lbd)):
    ...
    ny, nx = conv_adi_coro.shape
    e = [-ny/2*pixscale, nx/2*pixscale, -ny/2*pixscale, nx/2*pixscale]
    axs[i][0].imshow(conv_adi_coro+conv_adi_pl, cmap="Blues_r", origin='lower', 
               extent = e) #, vmin=-1e4, vmax=1e6)
    #axs[i][0].suptitle(f"{star_name}: {coronagraph} {quartile} ADI image at $\lambda$={lbd[i]:5.3f}, pxscale={pixscale_in_mas}, Mag {star_mag}",fontsize=12)
    axs[i][0].set_title('Planet: {} R_Jup, {} pc, {}K, logg={}, met={:.2f}, CO ratio={:3.2f}'.format(r_pl, dist_pc, temp, logg, met, CO),fontsize=10)
    axs[i][0].set_xlabel('[arcsec]')
    axs[i][0].set_ylabel('[arcsec]')
...
#     plt.clf()
    axs[i][1].plot(x, rms_contrast, label="Lambda={:5.3f} um".format(lbd[i]))

    
    #axs[i][1].suptitle(f"{star_name}: {coronagraph} {quartile} Contrast Profile of $\lambda$={lbd[i]:5.3f}, pxscale={pixscale_in_mas}, Mag {star_mag}",fontsize=12)
    axs[i][1].set_title('Planet: {} R_Jup, {} pc, {}K, logg={}, met={:.2f}, CO ratio={:3.2f}'.format(r_pl, dist_pc, temp, logg, met, CO),fontsize=10)
    axs[i][1].set_xlabel("Distance from center (mas)")
    axs[i][1].set_ylabel("Contrast, 5-sigma")
    axs[i][1].set_yscale('log')
    axs[i][1].grid(color='.9')

    #for j,d in enumerate(pl_dist):
    axs[i][1].plot(pl_dist, max_planet, 'o')
plt.show()'''
